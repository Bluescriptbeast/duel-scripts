...
local function showDiscordInProgressBar()
	...
	progressPct.Text = "discord.gg/meowlhub"
...
for _, name in pairs({"MeowlHubGUI"}) do
...
local _C={
	[1]=Color3.fromRGB(5,15,5),
	[2]=Color3.fromRGB(8,20,8),
	[3]=Color3.fromRGB(12,30,12),
	[4]=Color3.fromRGB(18,50,18),
	[5]=Color3.fromRGB(0,180,80),
	[6]=Color3.fromRGB(0,255,120),
	[7]=Color3.fromRGB(255,255,255),
	[8]=Color3.fromRGB(170,255,170),
	[9]=Color3.fromRGB(20,80,20),
	[10]=Color3.fromRGB(8,25,8),
}
...
topbar.BackgroundColor3 = Color3.fromRGB(0, 45, 15)
...
verLbl.Text = "discord.gg/meowlhub"
...
minBtn.BackgroundColor3 = Color3.fromRGB(0, 80, 30)
...
minBtn.MouseEnter:Connect(function() TweenService:Create(minBtn, TweenInfo.new(0.1), {BackgroundColor3=Color3.fromRGB(0,140,60)}):Play() end)
...
do local _st=Instance.new("Frame",main); _st.Size=UDim2.new(0,SW,0,CORNER); _st.Position=UDim2.new(0,0,0,44); _st.BackgroundColor3=SIDEBAR_BG; _st.BorderSizePixel=0; _st.ZIndex=4 end
...
mini.BackgroundColor3 = Color3.fromRGB(0,45,15)
...
mini.MouseEnter:Connect(function() TweenService:Create(mini,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(0,120,50)}):Play() end)
...
local ACTIVE_TAB_BG  = Color3.fromRGB(0,180,80)
local ACTIVE_TAB_TXT = WHITE
local IDLE_TAB_BG    = Color3.fromRGB(10,30,10)
...
pbFrame.BackgroundColor3 = Color3.fromRGB(5, 20, 5)
pbFrame.BorderSizePixel = 0
pbFrame.Active = true
Instance.new("UICorner", pbFrame).CornerRadius = UDim.new(0, 10)
Instance.new("UIStroke", pbFrame).Color = Color3.fromRGB(0, 255, 120)
...
pbBg.BackgroundColor3 = Color3.fromRGB(10, 40, 10)
...
do
	...
	local function playPreview(idx)
		...
		task.spawn(function()
			pcall(function()
				local tempFile = "MeowlHubPreview"
    ...
    getgenv().MeowlMusicBtn = musicBtn
...
do
	...
	local Q_OFF      = Color3.fromRGB(10, 35, 10)
	local Q_ON       = Color3.fromRGB(0, 255, 120)
	local Q_TEXT_OFF = Color3.fromRGB(200, 255, 200)
	local Q_TEXT_ON  = Color3.fromRGB(0, 40, 0)
	...
	local function createMobileButton(name, displayText, col, row, isToggle, onAction)
		...
		local function flash()
			TweenService:Create(btn, TweenInfo.new(0.08), {BackgroundColor3=Color3.fromRGB(0,255,120), TextColor3=Q_TEXT_ON}):Play()
...
saveConfig = function(btn)
	...
	local ok = pcall(function()
		...
		if writefile then writefile("MeowlMobileConfig.json", encoded) end
	end)
	if not ok then
		pcall(function()
			local encoded = HttpService:JSONEncode(cfg)
			if _writefile then _writefile("MeowlMobileConfig.json", encoded) end
		end)
	end
	if btn then
		local prev = btn.Text
...
local CONFIG_FILE="MeowlMobileConfig.json"
...
local Presets={}
local PRESET_FILE="MeowlMobilePresets.json"; local LAST_PRESET_FILE="MeowlMobileLastPreset.json"
...
loadConfig=function()
   ...
   if cfg.selectedIntroMusic then 
    ...
    task.defer(function() 
        if getgenv().MeowlMusicBtn then 
            getgenv().MeowlMusicBtn.Text = "Music " .. State.selectedIntroMusic 
        end 
    end)
end
                ...
                for _, child in ipairs(page:GetChildren()) do
                    if child:IsA("Frame") then
                        for _, subchild in ipairs(child:GetChildren()) do
                            if subchild:IsA("TextBox") and subchild.Parent.Name ~= "MeowlHubGUI" then
...
local function setupOtherPlayerBillboard(player)
    ...
    local function addBillboard(char)
        ...
        local oldBB = head:FindFirstChild("MeowlOtherBB")
        if oldBB then oldBB:Destroy() end
        
        local bb = Instance.new("BillboardGui", head)
        bb.Name = "MeowlOtherBB"
...
local function setupChar(char)
    ...
    local head=char:FindFirstChild("Head")
    if head then
        local oldBB=head:FindFirstChild("MeowlMobileBB"); if oldBB then oldBB:Destroy() end
        local bb=Instance.new("BillboardGui",head); bb.Name="MeowlMobileBB"
        ...
        discordLbl.BackgroundTransparency=1; discordLbl.Text="discord.gg/meowlhub"
...
local function playIntroAnimation()
	...
	task.spawn(function()
		pcall(function()
			...
			writefile("MeowlHubIntro", game:HttpGet(RandomIntro))
			local flex1 = Instance.new("Sound", gethui())
			flex1.SoundId = getcustomasset("MeowlHubIntro")
